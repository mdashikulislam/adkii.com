...
class CategorySeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($entries as $entryData) {
            $subEntries = $entryData['children'] ?? [];
            unset($entryData['children']);
            $parent = Category::where('slug', $entryData['slug'])->first();
            if ($parent) {
                $parent->update($entryData);
            } else {
                $parent = Category::create($entryData);
            }
            if (!empty($subEntries)) {
                foreach ($subEntries as $childData) {
                    $child = Category::where('slug', $childData['slug'])->first();
                    if ($child) {
